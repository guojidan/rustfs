...
impl AdaptiveMonitor {
    ...
    fn get_metrics(&self) -> AdaptiveMetrics {
        ...
        AdaptiveMetrics {
            ...
            avg_latency: if ops > 0 {
                Duration::from_millis(latency_sum / ops)
            } else {
                Duration::ZERO
            },
...
impl AdaptiveTestEnv {
    ...
    async fn prepare_data(&self, bucket: &str, count: usize) -> anyhow::Result<()> {
        ...
        for i in 0..count {
            ...
            self.ecstore
                .put_object(bucket, &object_name, &mut reader, &Default::default())
                .await?;
        }
        Ok(())
    }
    ...
    async fn run_load_scenario(&self, scenario: LoadScenario, monitor: Arc<AdaptiveMonitor>) -> anyhow::Result<()> {
        ...
        for phase in scenario.phases {
            ...
            while start.elapsed() < phase.duration {
                ...
                let _result = self
                    .ecstore
                    .get_object_info(bucket, &format!("test-{:06}", obj_id), &Default::default())
                    .await;
...
async fn benchmark_adaptive_scenario(
    env: &AdaptiveTestEnv,
    scenario: LoadScenario,
    monitor: Arc<AdaptiveMonitor>,
) -> anyhow::Result<AdaptiveMetrics> {
