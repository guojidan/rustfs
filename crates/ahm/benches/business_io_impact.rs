...
impl TestEnvironment {
    ...
    async fn create_test_ecstore(temp_dir: &TempDir) -> anyhow::Result<Arc<ecstore::store::ECStore>> {
        // 创建测试磁盘路径
        let disk_paths = (0..4).map(|i| temp_dir.path().join(format!("disk{}", i))).collect::<Vec<_>>();
...
async fn benchmark_get_operations(
    ...
    for i in 0..concurrency {
        ...
        let handle = tokio::spawn(async move {
            ...
            match ecstore
                .as_ref()
                .get_object_info(bucket_name, &object_name, &Default::default())
                .await
            {
                Ok(_) => {
                    monitor_clone.record_io_operation();
...
criterion_group!(benches, benchmark_business_io_impact, benchmark_latency_distribution,);
