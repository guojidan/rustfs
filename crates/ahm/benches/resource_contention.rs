...
use rustfs_ecstore::store_api::{MakeBucketOptions, ObjectIO, ObjectOptions, PutObjReader};
use rustfs_ecstore::{
...
impl ResourceContentionMonitor {
    ...
    fn update_system_metrics(&mut self) {
        ...
        while cpu_usage > current_cpu {
            match self
                .peak_cpu_usage
                .compare_exchange_weak(current_cpu, cpu_usage, Ordering::Relaxed, Ordering::Relaxed)
            {
                Ok(_) => break,
        ...
        while memory_usage > current_memory {
            match self
                .peak_memory_usage
                .compare_exchange_weak(current_memory, memory_usage, Ordering::Relaxed, Ordering::Relaxed)
            {
                Ok(_) => break,
            ...
            let handle = tokio::spawn(async move {
                ...
                while start_time.elapsed() < duration && !stop_signal.load(Ordering::Relaxed) {
                    ...
                    if is_put {
                        ...
                        if let Err(_) = ecstore
                            .put_object(bucket_name, &object_name, &mut put_reader, &Default::default())
                            .await
                        {
                            is_error = true;
                        }
                        ...
                        if let Err(_) = ecstore
                            .get_object_info(bucket_name, &existing_object, &Default::default())
                            .await
                        {
                            is_error = true;
                        }
...
async fn benchmark_business_scanner_contention(
    ...
    // 运行业务负载
    env.simulate_business_load(load_level, concurrent_requests, test_duration, monitor.clone())
        .await?;
                    ...
                    |b, (load, conc)| {
                        b.iter(|| {
                            rt.block_on(async {
                                ...
                                let metrics = benchmark_business_scanner_contention(&env, load.clone(), *conc, monitor)
                                    .await
                                    .unwrap();
        ...
        group.bench_function(format!("p95_latency_{}", config.name), |b| {
            b.iter(|| {
                rt.block_on(async {
                    ...
                    // 运行中等负载测试
                    let metrics = benchmark_business_scanner_contention(&env, BusinessLoadLevel::Medium, 10, monitor)
                        .await
                        .unwrap();
