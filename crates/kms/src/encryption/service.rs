...
impl ObjectEncryptionService {
    ...
    pub async fn decrypt_object(
        ...
        // Parse algorithm
        let algorithm = EncryptionAlgorithm::from_str(&metadata.algorithm)
            .ok_or_else(|| KmsError::unsupported_algorithm(&metadata.algorithm))?;
...
mod tests {
    ...
    async fn create_test_service() -> (ObjectEncryptionService, TempDir) {
        ...
        let backend = Arc::new(crate::backends::local::LocalKmsBackend::new(config.clone()).await.unwrap());
