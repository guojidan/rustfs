...
impl KmsCache {
    ...
    /// Create a new KMS cache with custom TTL settings
    pub fn with_ttl(capacity: u64, metadata_ttl: Duration, data_key_ttl: Duration) -> Self {
        Self {
    ...
    /// Put data key with specific key spec into cache
    pub async fn put_data_key_with_spec(&mut self, key_id: &str, plaintext: &[u8], ciphertext: &[u8], key_spec: KeySpec) {
        let cached_key = CachedDataKey {
            plaintext: plaintext.to_vec(),
            ciphertext: ciphertext.to_vec(),
            key_spec,
    ...
    pub fn info(&self) -> CacheInfo {
        CacheInfo {
            ...
            key_metadata_capacity: 50, // Hardcoded since moka doesn't expose max_capacity easily
            data_key_capacity: 50,
        }
    }

    /// Check if cache contains key metadata
    pub fn contains_key_metadata(&self, key_id: &str) -> bool {
        self.key_metadata_cache.contains_key(key_id)
    }

    /// Check if cache contains data key
    pub fn contains_data_key(&self, key_id: &str) -> bool {
...
pub struct CacheInfo {
    pub key_metadata_count: u64,
    pub data_key_count: u64,
    pub key_metadata_capacity: u64,
    pub data_key_capacity: u64,
}
...
impl CacheInfo {
    ...
    pub fn total_capacity(&self) -> u64 {
        self.key_metadata_capacity + self.data_key_capacity
    }
...
mod tests {
    ...
    async fn test_cache_operations() {
        ...
        assert_eq!(retrieved.unwrap().key_id, "test-key-1");
        ...
        let cached_data_key = cached_data_key.unwrap();
        ...
        // Test cache clearing
        cache.clear().await;
        let info_after_clear = cache.info();
    ...
    async fn test_cache_contains_methods() {
        ...
        assert!(!cache.contains_key_metadata("nonexistent"));
        assert!(!cache.contains_data_key("nonexistent"));
        ...
        assert!(cache.contains_key_metadata("contains-test"));
        assert!(cache.contains_data_key("contains-test"));
    }
}
